...
- (void)setItem:(PhotoGroupItem *)item {
    ...
    [self loadImageWithItem:item progress:^(NSInteger receivedSize, NSInteger expectedSize) {
       @strongify(self);
       if (!self) return;
    ...
    } completed:^(UIImage *image, BOOL isFinish, NSError *error) {
        @strongify(self);
        if (!self) return;
...
- (void)loadImageWithItem:(PhotoGroupItem *)item  progress:(void(^)(NSInteger receivedSize, NSInteger expectedSize))progress completed:(void(^)(UIImage *image, BOOL isFinish, NSError *error))completed
{
    BOOL useSD = NO;
    if ([_cellDelegate respondsToSelector:@selector(getImageLoadUseSDWeb)]) {
        useSD = [_cellDelegate getImageLoadUseSDWeb];
    }
    UIImage *placeholderImage = nil;
    if ([_cellDelegate respondsToSelector:@selector(cell:getImageWithPage:)]) {
        placeholderImage = [_cellDelegate cell:self getImageWithPage:_page];
    ...
    if (useSD) {
        [_imageView sd_setImageWithURL:item.largeImageURL placeholderImage:placeholderImage options:SDWebImageRetryFailed | SDWebImageRefreshCached progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
            if (progress) {
                progress(receivedSize, expectedSize);
            }
        } completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (completed) {
                completed(image, YES, error);
            }
        }];
    } else {
        [_imageView yy_setImageWithURL:item.largeImageURL placeholder:placeholderImage options:kNilOptions progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            ...
            if (completed) {
                completed(image, stage == YYWebImageStageFinished, error);
            }
        }];
    }
}
...
- (UIImage *)readCacheImageWithImageUrl:(NSURL *)url
{
    if (_useSDWeb) {
       return [[SDImageCache sharedImageCache] imageFromCacheForKey:url.absoluteString];
    } else {
        NSString *imageKey = [[YYWebImageManager sharedManager] cacheKeyForURL:url];
        return [[YYWebImageManager sharedManager].cache getImageForKey:imageKey withType:YYImageCacheTypeMemory];
    }
}
...
- (void)presentAnimated:(BOOL)animated completion:(void (^)(void))completion
{
    ...
    if (![item thumbClippedToTop:_fromView]) {
        if ([self readCacheImageWithImageUrl:item.largeImageURL]) {
            cell.item = item;
        }
    }
    if (!cell.item) {
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    if (fromView == nil) {
        ...
        [UIView animateWithDuration:animated ? 0.25 : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
            ...
            if (completion) completion();
            if (self.appearBlock) {
                self.appearBlock(self.fromNavigationBarHidden);
            }
        }];
        return;
    }
